...
async function finalForceSeed() {
  console.log('🔥 OMA Final Force Seed Engaged...');
  ...
  try {
    ...
    for (const draft of episodeDrafts) {
      console.log(`📝 Inserting EP ${draft.num}...`);
      const { data: epData, error: epErr } = await supabase
        .from('episodes_v2')
        .insert({
          project_id: PROJECT_ID,
          episode_number: draft.num,
          title: draft.title,
          summary: draft.summary,
          script_content: draft.num === 1 ? `
[SCENE 1: 아레나 코어 룸 - 밤]
...
시스템 경고음이 울린다. [ARENA CORE IGNITED]
      ...
      // 씬 데이터 (story_beats_v2) - 컬럼명 재확인 (scene_number, title, description, episode_id)
      const beats = [];
      for (let i = 0; i < 4; i++) {
        beats.push({
          episode_id: epData.id,
          scene_number: i + 1,
          title: `${draft.num}회 - 씬 ${i + 1}: ${['서막', '갈등', '전개', '절정'][i]}`,
          description: `에피소드 ${draft.num}의 핵심 전환점 ${i + 1}. 시나리오 엔진의 긴장감이 고조되는 장면이다.`,
          act_number: 1,
          beat_type: 'Scene',
          timestamp_label: `00:${String((i + 1) * 10).padStart(2, '0')}:00`
        });
      }

      const { error: beatErr } = await supabase.from('story_beats_v2').insert(beats);
      if (beatErr) console.error(`   ⚠️ Beat Sync Fail:`, beatErr.message);
      else console.log(`   🎬 Beats Synced.`);
    }

    await supabase.from('projects_v2').update({ episode_count: 6, status: 'READY' }).eq('id', PROJECT_ID);
    console.log('✨ OMA: Final Seed Success!');
  } catch (e) {
    console.error('❌ OMA Critical Failure:', e.message);
  }
}
