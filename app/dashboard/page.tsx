...
export default function DashboardPage() {
  ...
  // Simulation: Progress Baking Projects
  useEffect(() => {
    const interval = setInterval(() => {
      setProjects((prev) => 
        prev.map((p) => {
          if (p.status === 'BAKING' && p.progress < 100) {
            const nextProgress = p.progress + (100 / 15); // Simulate 15s completion
            if (nextProgress >= 100) {
  ...
  // Load Persistence (Projects + Step 6 Credits)
  useEffect(() => {
            ...
            <main className="flex-1 flex flex-col justify-center">
              <div className="px-10 mb-16">
                ...
                >
                  ...
                  {/* Step 6: Visual Credit Sync Indicator */}
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] font-black text-neutral-600 uppercase tracking-widest mb-1">Available Credits</span>
                    <span className="text-3xl font-black text-brand-gold italic tracking-tighter tabular-nums">
